 ...
 */
// 兼容 wechatpay-node-v3 不同版本的导出方式
const wechatpayModule = require('wechatpay-node-v3');
const Wechatpay = wechatpayModule.Wechatpay || wechatpayModule.default || wechatpayModule;
// 兼容 alipay-sdk 不同版本的导出方式
const alipayModule = require('alipay-sdk');
const AlipaySdk = alipayModule.AlipaySdk || alipayModule.default || alipayModule;
...
try {
  ...
  if (process.env.WX_APPID && process.env.WX_MCHID && privateKey && process.env.WX_APIV3_KEY) {
    wxpay = new Wechatpay({
    ...
    console.warn('[Pay] 微信支付未配置，支付功能不可用');
    wxpay = null;
  }
} catch (err) {
  console.warn('[Pay] 微信支付初始化失败:', err.message);
  wxpay = null;
}
...
try {
  ...
  if (process.env.ALIPAY_APPID && privateKey) {
    ...
    console.warn('[Pay] 支付宝未配置，支付功能不可用');
    alipaySdk = null;
  }
} catch (err) {
  console.warn('[Pay] 支付宝初始化失败:', err.message);
  alipaySdk = null;
}
