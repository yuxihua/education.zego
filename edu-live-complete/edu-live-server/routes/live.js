...
/**
 * @GET /api/live/token
 * 获取 ZEGO 加入房间的 Token（学生端使用）
 */
router.get('/token', asyncHandler(async (req, res) => {
  const { userID, roomID } = req.query;
  
  if (!userID || !roomID) {
    return fail(res, '缺少 userID 或 roomID 参数', 400, 400);
  }
  ...
  try {
    ...
    success(res, { token, userID, roomID });
  } catch (err) {
    console.error('生成 Token 失败:', err);
    // 如果工具不可用，返回模拟token（开发环境使用）
    if (process.env.NODE_ENV === 'development') {
      success(res, { 
        token: 'mock_token_' + Date.now(), 
        userID, 
        roomID,
        note: '模拟Token，生产环境需配置 ZEGO 密钥' 
      });
    } else {
      fail(res, 'Token 生成失败: ' + err.message, 500);
    }
  }
}));
...
/**
 * @POST /api/live/room/:id/heartbeat
 * 学生端心跳保活（防止被清理）
 */
router.post('/room/:id/heartbeat', asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { userId } = req.body;
  ...
  if (userId) {
    ...
    await redis.expire(`room:${id}:online`, 3600);
    // 刷新用户在线标记
    await redis.setex(`room:${id}:user:${userId}`, 60, Date.now());
  }

  success(res, { timestamp: Date.now() });
}));
