...
function buildAudienceUserId(req) {
  const role = String(req.user?.role || '').trim().toLowerCase();
  const studentId = Number(getCurrentStudentId(req) || 0);

  if (role === 'parent') {
    return studentId ? `parent_${studentId}` : `parent_guest_${Date.now()}`;
  }

  if (!role || role === 'student') {
    return studentId ? `student_${studentId}` : `student_guest_${Date.now()}`;
  }
...
async function checkStudentCourseAccess(req, res, courseId, options = {}) {
  ...
  if (!isStudentIdentity(req) && req.user?.role !== 'superadmin') {
    fail(res, '仅学员或家长可访问直播观看页', 403, 403);
    return null;
  }
  ...
  if (!order) {
    fail(res, '仅已购课程的学员或家长可进入直播', 403, 403);
    return null;
  }
...
router.post('/room/:id/online', auth, asyncHandler(async (req, res) => {
  ...
  const onlineUserId = String(userId || '').trim() || buildAudienceUserId(req);
  await redis.sadd(`room:${id}:online`, onlineUserId);
  await redis.expire(`room:${id}:online`, 3600);

  const audienceStats = await getRoomAudienceStats(id);
  await room.update({ onlineCount: audienceStats.onlineCount });

  // 更新峰值
  if (audienceStats.onlineCount > room.peakCount) {
    await room.update({ peakCount: audienceStats.onlineCount });
  }

  success(res, audienceStats);
}));
...
router.post('/room/:id/offline', auth, asyncHandler(async (req, res) => {
  ...
  const onlineUserId = String(userId || '').trim() || buildAudienceUserId(req);
  if (onlineUserId) {
    await redis.srem(`room:${id}:online`, onlineUserId);
  }

  const audienceStats = await getRoomAudienceStats(id);
  await LiveRoom.update({ onlineCount: audienceStats.onlineCount }, { where: { id } });
